...
public class NetController {
    ...
    public void DownloadSoundList() throws UnsupportedEncodingException {
        ...
        if(ir.zemestoon.relaxmelody.BillingManager.getInstance(activity).isPremiumActivated())url=url.replace("soundlist","soundlist_pro");
    ...
    public void DownloadMixedList() throws UnsupportedEncodingException {
        ...
        // Variable to store the file content
        final String[] fileContent = {""}; // Using array to allow modification in inner class
        if(ir.zemestoon.relaxmelody.BillingManager.getInstance(activity).isPremiumActivated())url=url.replace("mixedlist","mixedlist_pro");
